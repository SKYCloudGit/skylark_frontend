...
const Configure = () => {
  ...
  const fetchMeters = async () => {
    try {
      ...
      const res = await axios.get(
        `${BASE_URL}/data/meter/list/hierarchyDataId=${hierarchyDataId}`,
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      await axios.get(
        `${BASE_URL}/c2dmessages/publish?topic=${encodeURIComponent(
          topic
        )}&payload=${encodeURIComponent(finalPayload)}&qos=${qos}&retainRequired=${retainRequired}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
